...
export default function FlashcardApp() {
  ...
  const isCorrect = useMemo(() => {
    ...
    if (type === "tf")
      return typeof userAns === "boolean" ? userAns === !!current.answer : null;
    ...
    <div className="page">
      <div className="app">
        ...
        */}
        ...
        {/* KART */}
        <div className="card">
          {/* RESİM GÖSTERİM KISMI BURAYA EKLENİR! */}
          {current.image && <CardImage src={current.image} q={current.q} />}
        ...
        <div className="controls">
          <button onClick={prev} className="btn" aria-label="Önceki">
            ← Geri
          </button>
          <button onClick={next} className="btn" aria-label="Sonraki">
            İleri →
          </button>
        </div>
    ...
    <div>
      ...
      <ul className="choices">
        {choices.map((ch, i) => {
          ...
          const showReveal =
            reveal && isCorrect && picked !== null && !isPicked;
          const cls = [
            "choice",
          ...
          return (
            <li key={i}>
              ...
              >
                ...
                {isPicked && isCorrect && (
                  <span className="badge ok">Doğru</span>
                )}
                {isPicked && !isCorrect && (
                  <span className="badge no">Yanlış</span>
                )}
                {!isPicked && showReveal && (
                  <span className="badge ok ghost">Doğru cevap</span>
                )}
              </button>
            </li>
          );
...
function OpenCard({ answer, revealed, onToggle }) {
  return (
    <div>
      ...
      >
        <span className="pill">
          {revealed ? "Görünüyor" : "Gizli (Tıkla / Space)"}
        </span>
        <div className={"open-answer " + (revealed ? "show" : "hide")}>
          {revealed ? answer : "••••••••"}
        </div>
      </button>
    </div>
...
function CardImage({ src, q }) {
  // Resim URL'si. Eğer bir URL değilse (örn. "my_image.png"), public klasöründen alınır.
  const imageUrl =
    src.startsWith("http") || src.startsWith("/") ? src : `/${src}`;
  console.log("Resim Yolu Kontrolü:", imageUrl);

  // Resim Açıklaması (Alternatif metin)
  const altText = q.split(".")[1]?.trim() || "Soruya ait görsel";
...
function Empty() {
  return (
    <div className="page">
      <div className="app">
        Kart yok. <code>public/questions.json</code> ekleyin.
      </div>
    </div>
  );
}
...
function inferType(card) {
  if (!card) return "open";
  if (Array.isArray(card.choices) && Number.isInteger(card.answerIndex))
    return "mcq";
...
// GÜNCELLENMİŞ normalizeCards fonksiyonu
function normalizeCards(cards) {
  return cards.map((c) => {
    const t = c.type || inferType(c);
    // 'image' alanını karta ekle, yoksa null olsun
    const base = { q: c.q ?? "—", image: c.image ?? null };
    if (t === "mcq") {
      return {
        type: "mcq",
        ...base, // <<< Temel alanları (q ve image dahil) yay
        choices: c.choices ?? [],
        answerIndex: Number(c.answerIndex ?? 0),
      };
    } else if (t === "tf") {
      return { type: "tf", ...base, answer: !!c.answer };
    } else {
      return { type: "open", ...base, a: String(c.a ?? "") };
    }
  });
}
